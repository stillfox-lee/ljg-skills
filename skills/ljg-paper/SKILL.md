...
输出结构依据 `references/template.md`。禁止参考当前工作目录中已有论文文件的章节结构——旧文件可能使用过期模板。
...
如果论文有一张承载全文核心思路的总览图（overview / architecture diagram，通常是 Figure 1），提取并保存到当前工作目录的 `./images/` 子目录，文件名 `{identifier}--paper-{简短标题}-overview.png`。
...
按 Denote 规范获取时间戳，读 `references/template.md`，写入当前工作目录。
